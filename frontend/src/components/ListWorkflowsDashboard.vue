<template>
  <v-container>
      ...
      <v-col cols="12">
        <v-card>
          <v-toolbar color="primary" dark>
            <v-card-title>最近流程</v-card-title>
            <v-spacer />
            <v-btn icon @click="confirmDialog = true" color="white" class="ml-2"
              :disabled="selectedFolders.length === 0 || isLoading || isDeleting" :loading="isDeleting">
          ...
          <v-data-table hide-default-footer :items-per-page="5" v-model="selectedFolders" :headers="headers"
            :items="folders" :loading="isLoading" item-value="name" :sort-by="sortBy" show-select dense
            class="elevation-1" no-data-text="No workflows available.">
    ...
    <!-- 打开工作流对话框 -->
    <v-dialog v-model="WorkflowDialog" width="90%" height="90%">
      <v-card>
        <v-toolbar color="primary" dark>
        ...
        <div class="workflow" style="width: 100%; height: 100%;">
          <OpenWorkflow 
          :workflowUuid="selectedWorkflowUuid"
          @updateWorkflowDialog="WorkflowDialog = $event"
          />
        </div>
      </v-card>
    </v-dialog>
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchFolders = async () => {
      try {
        const response = await axios.get('/api/dashboard_results');
        if (response.data.success) {
